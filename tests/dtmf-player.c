...
static void
deferred_cb (GabbleDTMFPlayer *dtmf_player G_GNUC_UNUSED,
    const gchar *tones,
    Fixture *f)
{
    ...
    gconstpointer nil G_GNUC_UNUSED)
{
  ...
  ok = gabble_dtmf_player_play (f->dtmf_player, "#p*", 1, 1, 10000, &f->error);
  g_assert_no_error (f->error);
  g_assert (ok);
  g_assert (gabble_dtmf_player_is_active (f->dtmf_player));
  fixture_assert_log (f, "started '#'\n");
    ...
    gconstpointer nil G_GNUC_UNUSED)
{
  ...
  ok = gabble_dtmf_player_play (f->dtmf_player, "*w#w", 1, 1, 1,
      &f->error);
  g_assert_no_error (f->error);
  g_assert (ok);
  g_assert (gabble_dtmf_player_is_active (f->dtmf_player));
  ...
  ok = gabble_dtmf_player_play (f->dtmf_player, "#w", 1, 1, 1,
      &f->error);
  g_assert_no_error (f->error);
  g_assert (ok);
  g_assert (gabble_dtmf_player_is_active (f->dtmf_player));
